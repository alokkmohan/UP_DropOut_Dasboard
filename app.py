...
# ==================== TAB 4: SCHOOL PERFORMANCE ANALYSIS ====================
    ...
    with col_f3:
        ...
        # Get all schools based on filters
        if school_year == "All" and school_selected_district == "All":
            schools_query = f'SELECT DISTINCT "Last School Name" FROM "{csv_file}" WHERE "Last School Name" IS NOT NULL AND "Last School Name" != \'\' ORDER BY "Last School Name"'
        elif school_year == "All":
            schools_query = f'SELECT DISTINCT "Last School Name" FROM "{csv_file}" WHERE "District Name" = \'{school_selected_district}\' AND "Last School Name" IS NOT NULL AND "Last School Name" != \'\' ORDER BY "Last School Name"'
        elif school_selected_district == "All":
            schools_query = f'SELECT DISTINCT "Last School Name" FROM "{csv_file}" WHERE "Academic Year" = \'{school_year}\' AND "Last School Name" IS NOT NULL AND "Last School Name" != \'\' ORDER BY "Last School Name"'
        else:
            schools_query = f'SELECT DISTINCT "Last School Name" FROM "{csv_file}" WHERE "Academic Year" = \'{school_year}\' AND "District Name" = \'{school_selected_district}\' AND "Last School Name" IS NOT NULL AND "Last School Name" != \'\' ORDER BY "Last School Name"'
    ...
    if selected_school != "-- Select School --":
        with st.spinner('📊 Loading school data...'):
            try:
                # Get school data
                district_filter = f'AND "District Name" = \'{school_selected_district}\'' if school_selected_district != "All" else ''
                
                if school_year == "All":
                    school_data_query = f'''
                        SELECT *
                        FROM "{csv_file}"
                        WHERE "Last School Name" = '{selected_school}'
                        {district_filter}
                    '''
                else:
                    school_data_query = f'''
